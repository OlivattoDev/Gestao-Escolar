...
import Application.Periodo;
import Application.Turma;
...
public class CadastroDeTurma {
	...
	public void cadastrar(String codigo, LocalDate dataInicio, LocalDate dataFim, Periodo periodo, String codigoCurso) {
		...
		try {
			Files.writeString(arquivo, codigo + "," + dataInicio.format(mascaraData) + "," + dataFim.format(mascaraData)
	...
	public List<Turma> listar() {
		...
		try {
			...
			for (var linha : linhas) {
				...
				var turma = new Turma(campos[0], curso, LocalDate.parse(campos[1], mascaraData),
						LocalDate.parse(campos[2], mascaraData), Periodo.valueOf(campos[3]));
				lista.add(turma);
			}
